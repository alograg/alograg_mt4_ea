...
int start() {
  ...
  if (AccountFreeMargin() < free * 1500.0)
    return (0);

  if (MM) {
    if (LotsPercent > 0)
      Lots = NormalizeDouble(
          MathCeil(AccountFreeMargin() / 10000.0 * LotsPercent) / 10.0, 1);
    else
      Lots = Lots;
  }
  ...
  if ((isMgNum(Magic) == 0) && (RSI < RSI_BUY)) {
    if (Compra)
      OrderSend(Symbol(), OP_BUY, Lots, Ask, 4, Ask - SL * Point,
                Ask + TP * Point, 0, Magic, 0, Green);
    Compra = false;
    Venda = true;
  }
  ...
  if ((isMgNum(Magic) == 0) && (RSI > RSI_SELL)) {
    if (Venda)
      OrderSend(Symbol(), OP_SELL, Lots, Bid, 4, Bid + SL * Point,
                Bid - TP * Point, 0, Magic, 0, Red);
    Venda = false;
    Compra = true;
  }
