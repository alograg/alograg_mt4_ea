...
int start()
{
  ...
  {
     int result=0;
     if((High[1]<iMA(NULL,0,14,0,MODE_SMA,PRICE_CLOSE,1))&&(iRSI(NULL,0,14,PRICE_CLOSE,1)<70)) // Here is your open buy rule
     {
  ...
  for(int cnt=0;cnt<OrdersTotal();cnt++)
     {
      ...
      if(OrderType()<=OP_SELL &&   
         ...
         )  
        {
         if(OrderType()==OP_BUY)  
           {
              if((iRSI(NULL,0,14,PRICE_CLOSE,1)>70)&&(Close[1]>iMA(NULL,PERIOD_M15,14,0,MODE_SMA,PRICE_HIGH,1))) //here is your close buy rule
              {
