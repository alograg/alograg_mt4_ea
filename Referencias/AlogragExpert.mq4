...
void OnTick() {
  ...
  double MaLowE =
      iMA("EURGBP", PERIOD_M5, EvaluatePeriods, 0, MODE_EMA, PRICE_LOW, 0);
  double MaHighE =
      iMA("EURGBP", PERIOD_M5, EvaluatePeriods, 0, MODE_EMA, PRICE_HIGH, 0);
  double MaAvgE =
      iMA("EURGBP", PERIOD_M5, EvaluatePeriods, 0, MODE_EMA, PRICE_TYPICAL, 0);
  double MaLowELast =
      iMA("EURGBP", PERIOD_M5, EvaluatePeriods, 0, MODE_EMA, PRICE_LOW, 1);
  double MaHighELast =
      iMA("EURGBP", PERIOD_M5, EvaluatePeriods, 0, MODE_EMA, PRICE_HIGH, 1);
  double MaAvgELast =
      iMA("EURGBP", PERIOD_M5, EvaluatePeriods, 0, MODE_EMA, PRICE_TYPICAL, 1);
  double MaLow =
      iMA("EURGBP", PERIOD_M5, EvaluatePeriods, 0, MODE_SMA, PRICE_LOW, 0);
  double MaHigh =
      iMA("EURGBP", PERIOD_M5, EvaluatePeriods, 0, MODE_SMA, PRICE_HIGH, 0);
  double MaLowLast =
      iMA("EURGBP", PERIOD_M5, EvaluatePeriods, 0, MODE_SMA, PRICE_LOW, 1);
  double MaHighLast =
      iMA("EURGBP", PERIOD_M5, EvaluatePeriods, 0, MODE_SMA, PRICE_HIGH, 1);
  if (TotalOrdersCount() <= 10) {
    int result = 0;
    // El minimo ponderado crusa el minimo exponencial
    // Print("Aqui: ", MaLowLast, MaLowELast, MaLow, MaLowE);
    bool isDown =
        (MaLowLast >= MaLowELast && MaLow <= MaLowE && High[0] <= MaAvgE) ||
        (Open[1] <= MaLowLast && High[1] > MaHigh);
    bool isUp = Close[1] >= MaHighELast && Open[1] >= MaHighELast &&
                Close[0] >= MaHighE && Open[0] >= MaHighE && MaLowE >= MaLow;
    bool canPlaceBuy =
        lastBuyTime != (int)Time[0] && TotalBuyOrdersCount() <= 4;
    bool canPlaceSell =
        lastBuyTime != (int)Time[0] && TotalSellOrdersCount() <= 4;
    ...
    if (isDown && canPlaceBuy) // Here is your open buy rule
    {
      ...
      result = OrderSend("EURGBP", OP_BUY, Lots, Ask, Slippage, 0, 0,
                         "Alograg propuso", MagicNumber, 0, Blue);
      if (result > 0) {
        TheStopLoss = 0;
        TheTakeProfit = 0;
        if (TakeProfit > 0)
        ...
        // hasOrder = OrderModify(OrderTicket(), OrderOpenPrice(),
        // NormalizeDouble(TheStopLoss, Digits), NormalizeDouble(TheTakeProfit,
        // Digits), 0, Green);
        lastBuyTime = Time[0];
      }
    }
    if (isUp && canPlaceSell) // Here is your open buy rule
    {
      ...
      result = OrderSend("EURGBP", OP_SELL, Lots, Bid, Slippage, 0, 0,
                         "Alograg propuso", MagicNumber, 0, Blue);
      if (result > 0) {
        TheStopLoss = 0;
        TheTakeProfit = 0;
  ...
  for (int cnt = 0; cnt < countOrders; cnt++) {
    ...
    if (isWorkingOrder) {
      ...
      bool hasPositiveProfit =
          OrderProfit() > ((1 + MinProfit) * OrderOpenPrice());
      bool canSell;
      if (isBuy) {
        canSell = (int)OrderOpenTime() != (int)Time[0] && hasPositiveProfit;
        bool lastLowOnTop = Low[1] >= MaHighLast && Low[1] >= MaAvgELast &&
                            Between(Low[0], MaHighE, MaAvgE);
        // Print("Evaluar",canSell, lastLowOnTop);
        if (canSell && lastLowOnTop) {
          hasOrder = OrderClose(OrderTicket(), OrderLots(), OrderClosePrice(),
                                Slippage, Red);
      ...
      if (isSell) {
        bool canClose =
            (int)OrderOpenTime() != (int)Time[0] && OrderProfit() > 0.5;
        bool lastLowOnLow =
            Close[1] <= MaLowELast && High[0] < MaLow && MaLowE > MaLowELast;
        // Print("Evaluar",canSell, lastLowOnTop);
