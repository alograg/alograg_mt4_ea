...
void OnInit() { lastBuyTime = Time[0]; }
//+------------------------------------------------------------------+
...
void OnStart() {
  ...
  double MaLowE =
      iMA("EURGBP", PERIOD_M1, EvaluatePeriods, 0, MODE_EMA, PRICE_LOW, 0);
  double MaHighE =
      iMA("EURGBP", PERIOD_M1, EvaluatePeriods, 0, MODE_EMA, PRICE_HIGH, 0);
  double MaAvgE =
      iMA("EURGBP", PERIOD_M1, EvaluatePeriods, 0, MODE_EMA, PRICE_TYPICAL, 0);
  double MaLowELast =
      iMA("EURGBP", PERIOD_M1, EvaluatePeriods, 0, MODE_EMA, PRICE_LOW, 1);
  double MaHighELast =
      iMA("EURGBP", PERIOD_M1, EvaluatePeriods, 0, MODE_EMA, PRICE_HIGH, 1);
  double MaAvgELast =
      iMA("EURGBP", PERIOD_M1, EvaluatePeriods, 0, MODE_EMA, PRICE_TYPICAL, 1);
  double MaLow =
      iMA("EURGBP", PERIOD_M1, EvaluatePeriods, 0, MODE_SMA, PRICE_LOW, 0);
  double MaHigh =
      iMA("EURGBP", PERIOD_M1, EvaluatePeriods, 0, MODE_SMA, PRICE_HIGH, 0);
  double MaLowLast =
      iMA("EURGBP", PERIOD_M1, EvaluatePeriods, 0, MODE_SMA, PRICE_LOW, 1);
  double MaHighLast =
      iMA("EURGBP", PERIOD_M1, EvaluatePeriods, 0, MODE_SMA, PRICE_HIGH, 1);
  if (TotalOrdersCount() <= 4) {
    ...
    if (isDown && canBuy) // Here is your open buy rule
    {
      ...
      result =
          OrderSend("EURGBP", OP_BUY, Lots, Ask, Slippage, 0, Ask * MinProfit,
                    "Alograg Compro", MagicNumber, 0, Blue);
      if (result > 0) {
        TheStopLoss = 0;
        TheTakeProfit = 0;
        if (TakeProfit > 0)
  ...
  for (int cnt = 0; cnt < countOrders; cnt++) {
    ...
    if (isWorkingOrder) {
      ...
      bool lastHighFlagGood =
          Between(MaHighLast, Open[1], Close[1]) && MaHighELast < Close[1];
      ...
      if (isBuy && hasPositiveProfit) {
        Print("Evaluar", isBuy, hasPositiveProfit, lastHighFlagGood,
              currentHighFlagGoBad, canSell, noLoseBuy);
