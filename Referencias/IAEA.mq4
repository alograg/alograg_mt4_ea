...
int start() {
      ...
      if (OrderType() == OP_BUY) {
        ...
        if (Bid > (OrderStopLoss() + (sl * 2 + spread) * Point)) {
          if (perceptron() < 0) { // reverse
            ticket =
                OrderSend(Symbol(), OP_SELL, lots * 2, Bid, 3, Ask + sl * Point,
                          Ask - TakeProfitX * Point, "AI", MagicNumber, 0, Red);
            Sleep(30000);
            //----
            if (ticket < 0)
              prevtime = Time[1];
        ...
        if (Ask < (OrderStopLoss() - (sl * 2 + spread) * Point)) {
          if (perceptron() > 0) { // reverse
            ticket = OrderSend(Symbol(), OP_BUY, lots * 2, Ask, 3,
                               Bid - sl * Point, Bid + TakeProfitX * Point,
  ...
  if (perceptron() > 0) { // long
    ticket = OrderSend(Symbol(), OP_BUY, lots, Ask, 3, Bid - sl * Point,
                       Bid + TakeProfitX * Point, "AI", MagicNumber, 0, Blue);
    //----
    if (ticket < 0) {
      Sleep(30000);
      prevtime = Time[1];
    ...
    ticket = OrderSend(Symbol(), OP_SELL, lots, Bid, 3, Ask + sl * Point,
                       Ask - TakeProfitX * Point, "AI", MagicNumber, 0, Red);
    if (ticket < 0) {
      Sleep(30000);
      prevtime = Time[1];
