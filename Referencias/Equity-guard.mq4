...
if ( AccountEquity() <= secure_Min_balance ){CloseAll();}
int  dow = Day(); 
if(oldday != dow ){
todaybalance= AccountEquity();oldday= dow;
force=1;
SendMail("Daily Balance ", StringConcatenate("Equity : " , AccountEquity(), "Daily Level " , todaybalance - daily_quota, "All time Level :" , secure_Min_balance ));
}
if ( AccountEquity() <= (todaybalance - daily_quota)){ CloseAll();}

    Comment ("Equity : " , AccountEquity(), " Daily Level : " , (todaybalance - daily_quota), " All time Level : " ,secure_Min_balance );
//----
   return(0);
  }
//+------------------------------------------------------------------+
...
{  
   ...
   if(force > 0) {SendMail("Force Close ", StringConcatenate("Equity : " , AccountEquity(), " Daily Level: " , todaybalance - daily_quota, " All time Level : " , secure_Min_balance ));force=0;}
}
//+------------------------------------------------------------------+
