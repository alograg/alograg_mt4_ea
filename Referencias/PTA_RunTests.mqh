...
void RunTests() {
  ...
  double nextLevelTestValues[] = {
    110.80,  110.00,  110.01,  110.05,  110.10,  110.19,  110.20,  110.21,
    110.25,  110.39,  110.40,  110.49,  110.50,  110.51,  110.60,  110.70,
    110.799, 110.80,  110.801, 110.90,  110.999, 111,     1.1400,  1.14001,
    1.1410,  1.14199, 1.1420,  1.14201, 1.1430,  1.14499, 1.14500, 1.14501,
    1.1460,  1.1470,  1.14799, 1.1480,  1.14801, 1.1490,  1.14999, 1.1500
  };
  double nextLevelUpTestResults[] = {
    111.00,  110.20, 110.20,  110.20, 110.20,  110.20, 110.50, 110.50,
    110.50,  110.50, 110.50,  110.50, 110.80,  110.80, 110.80, 110.80,
    110.800, 111.00, 111.000, 111.00, 111.000, 111.20, 1.1420, 1.1420,
    1.1420,  1.1420, 1.1450,  1.1450, 1.1450,  1.1450, 1.1480, 1.1480,
    1.1480,  1.1480, 1.1480,  1.1500, 1.1500,  1.1500, 1.1500, 1.1520
  };
  for (int ix = 0; ix < ArraySize(nextLevelTestValues); ix++) {
    if (ProperlyDetectsNextLevelUp(nextLevelTestValues[ix],
                                   nextLevelUpTestResults[ix]))
      ++testsPassed;
    totalTests++;
  }
...
bool CanDetectNoNewOrders() // Scenario 1
{
  ...
  OnTick();
  if (Assert(totalActiveTrades == 0,
             "Number of Open Trades increased with no new Trades"))
    return true;
  return false;
}
...
bool CanDetectNewPendingOrder() {
  ...
  OnTick();
  if (!Assert(totalActiveTrades == 1, "Number of Active Trades is not 1"))
    return false;
...
bool CanDetectMultipleNewPendingTrades() {
  ...
  OnTick();
  if (!Assert(totalActiveTrades == 2, "Number of active trades is not 2"))
    return false;
  if (!Assert(activeTrades[0].IsPending,
              "IsPending for first trade is not true"))
    return false;
  return (Assert(activeTrades[1].IsPending,
                 "IsPending for second trade is not true"));
}
...
bool CanDetectMultipleNewMixedPendingAndActiveTrades() {
  ...
  if (!Assert(totalActiveTrades == 2, "Number of active trades is not 2"))
    return false;
  if (!Assert(activeTrades[0].IsPending == false,
              "IsPending is set for first trade"))
    return false;
  return (Assert(activeTrades[1].IsPending,
                 "IsPending for second trade is not true"));
}
...
bool CanDetectPendingTradeGoneActive() {
  ...
  testBroker.OrdersToReturn[0].OrderType = OP_BUY;
  OnTick();
  if (!Assert(totalActiveTrades == 1, "Number of active trades is no longer 1"))
    return false;
...
bool CanDetectPendingTradeNowDeleted() {
  ...
  if (!Assert(totalActiveTrades == 0, "Number of Active Orders is not 0"))
    return false;
  if (!Assert(ArraySize(activeTrades) == 1, "activeTrades array size is not 1"))
    return false;
...
bool CanDetectFirstPendingTradeNowActiveFromAmongMultipleTrades() {
  ...
  DeleteAllObjects();
  OnTick();
  if (!Assert(totalActiveTrades == 2, "Number of Active Orders is not 2"))
    return false;
  if (!Assert(activeTrades[0].IsPending == false,
              "IsPending for first trade is set"))
    return false;
  if (!Assert(activeTrades[1].IsPending,
              "IsPending for first trade is not set"))
    return false;
  return Assert(!EntryArrowDoesNotExist(Prefix + "Entry"),
                "Entry arrow not created for pending order gone active");
}
// Scenario 10a
// 10. Multiple pending trades previously, 1 trade now active
bool CanDetectSecondPendingTradeNowActiveFromAmongMultipleTrades() {
  Print(
      "Beginning CanDetectSecondPendingTradeNowActiveFromAmongMultipleTrades");
  FakeBroker *testBroker = broker;
  InitializeActiveTradeArray();
  ClearGlobalVariables();
  string gvName = GVPrefix + "1LastOrderId";
  ...
  if (!Assert(activeTrades[0].IsPending,
              "IsPending for first trade is not set"))
    return false;
  return (Assert(activeTrades[1].IsPending == false,
                 "IsPending for first trade is set"));
}
// Scenario 11
// 11. Multiple pending trades previously, multiple trades now active
...
bool CanDetectActiveTradeBeingClosed() {
  ...
  if (!Assert(totalActiveTrades == 0, "Number of Active Orders is not 0"))
    return false;
  if (!Assert(ArraySize(activeTrades) == 1, "activeTrades array is not size 1"))
    return false;
  return Assert(activeTrades[0] == NULL,
                "First entry in activeTrades is not null");
}
...
bool CanDetectMultipleActiveTradesWithFirstOneBeingClosed() {
  ...
  testBroker.OrdersToReturn[0].OrderClosed = TimeCurrent();
  OnTick();
  if (!Assert(totalActiveTrades == 1, "Number of open orders is not 1)"))
    return false;
  if (!Assert(ArraySize(activeTrades) == 2, "activeOrder array size is not 2"))
    return false;
  if (!Assert(CheckPointer(activeTrades[0]) == POINTER_DYNAMIC,
              "first active order is not a valid pointer"))
    return false;
...
bool CanDetectMultipleActiveTradesWithAllTradesBeingClosed() {
  ...
  testBroker.OrdersToReturn[1].OrderClosed = TimeCurrent();
  OnTick();
  if (!Assert(totalActiveTrades == 0, "Number of open orders is not 0)"))
    return false;
  if (!Assert(ArraySize(activeTrades) == 2, "activeOrder array size is not 2"))
    return false;
  if (!Assert(activeTrades[0] == NULL, "first active order is not null"))
    return false;
  return Assert(activeTrades[1] == NULL, "second active order is not null");
}
...
bool DetectingNewPendingOrderDoesNotInvokeNewTradeProcessing() {
  ...
  OnTick();
  return Assert(EntryArrowDoesNotExist(Prefix + "Entry"),
                "Trade Entry was called for pending order");
}

bool DetectingDeletedPendingOrderDoesNotInvokeClosedTradeProcessing() {
  Print("Beginning "
        "DetectingDeletedPendingOrderDoesNotInvokeClosedTradeProcessing");
  FakeBroker *testBroker = broker;
  InitializeActiveTradeArray();
  ClearGlobalVariables();
  string gvName = GVPrefix + "1LastOrderId";
  ...
  DeleteAllObjects();
  OnTick();
  return Assert(EntryArrowDoesNotExist(Prefix + "Exit"),
                "Old Trade Closed was called for deleted pending order");
}

bool HandlingContinuingActiveTradeDoesNotInvokeClosedTradeProcessing() {
  Print("Beginning "
        "HandlingContinuingActiveTradeDoesNotInvokeClosedTradeProcessing");
  FakeBroker *testBroker = broker;
  InitializeActiveTradeArray();
  ClearGlobalVariables();
  string gvName = GVPrefix + "1LastOrderId";
  ...
  OnTick();
  return Assert(EntryArrowDoesNotExist(Prefix + "Exit"),
                "Exit arrow was created for continuing active trade");
}
bool EntryArrowDoesNotExist(string objectNameToMatch) {

  bool foundArrow = false;
  for (int ix = 0; ix < ObjectsTotal(OBJ_ARROW); ix++) {
    if (StringSubstr(ObjectName(ix), 0, StringLen(objectNameToMatch)) ==
        objectNameToMatch) {
      foundArrow = true;
      break;
    }
...
bool ProperlyDetectsNextLevelUp(double currentPrice, double expected) {
  ...
  return (Assert(NormalizeDouble(nextLevel, 5) == expected,
                 "NextLevel Up for " + DoubleToStr(currentPrice, 5) +
                     " failed. Expeccted = " + DoubleToStr(expected, 8) +
                     " actual = " + DoubleToStr(nextLevel, 8)));
}

void SetNoTradesPreviously() // Set up the precondition that there are no trades
                             // outstanding that we know about
{
  InitializeActiveTradeArray();
}
...
void
ClearGlobalVariables() // Set up the test condition that there are no new orders
{
  int seqNo = 1;
  while (true) {
