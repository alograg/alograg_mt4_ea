...
int start() {
  ...
  if (TotalOrdersCount() == 0) {
    int result = 0;
    if ((Hour() < 5)) // Here is your open buy rule
    {
      result =
    ...
    if ((Hour() < 5)) // Here is your open Sell rule
    {
      result =
  ...
  for (int cnt = 0; cnt < OrdersTotal(); cnt++) {
    ...
    if (OrderType() <= OP_SELL && OrderSymbol() == Symbol() &&
      ...
      if (OrderType() == OP_BUY) {
        if ((Close[0] > Ask)) // here is your close buy rule
        {
        ...
        if ((Close[0] < Bid)) // here is your close sell rule
        {
          OrderClose(OrderTicket(), OrderLots(), OrderClosePrice(), Slippage,
                     Red);
        }
