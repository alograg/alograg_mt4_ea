...
bool isToUp()
{
    ...
    double yellowLowClose = iMA(Symbol(), PERIOD_M1, 10, 0, MODE_SMA, PRICE_LOW, 1);
    double greenOpen = iMA(Symbol(), PERIOD_M1, 10, 0, MODE_EMA, PRICE_HIGH, 0);
    double yellowHighOpen = iMA(Symbol(), PERIOD_M1, 10, 0, MODE_SMA, PRICE_HIGH, 0);
    bool fromDown = preLastClose < redClose && lastClose < redClose && preLastClose < yellowLowClose && lastClose < yellowLowClose;
    bool goUp = currentClose > greenOpen && currentClose > yellowHighOpen && greenOpen > yellowHighOpen;
...
bool isToDown()
{
    double currentClose = Close[0];
    double lastClose = Close[1];
    double greenOpenLast = iMA(Symbol(), PERIOD_M15, 10, 0, MODE_EMA, PRICE_HIGH, 1);
    double yellowHighOpenLast = iMA(Symbol(), PERIOD_M15, 10, 0, MODE_SMA, PRICE_HIGH, 1);
...
{
    return OrderSendReliable(Symbol(), type, getLotSize(), price, 3, 0, 0, eaName + ": Crossing." + IntegerToString(type), MagicNumber, 0, Blue);
}
...
void exitBuys()
{
    ...
    for (int i = 0; i < totalOrders; i++)
    {
        ...
        if (OrderSymbol() == Symbol() && isFornComment(comment) && profit > 0 && OrderType() == OP_BUY)
        {
...
void exitSell()
{
    ...
    for (int i = 0; i < totalOrders; i++)
    {
        if (!OrderSelect(i, SELECT_BY_POS))
            continue;
        profit = NormalizeDouble(OrderProfit() + OrderCommission() + OrderSwap() - minCents, 2);
        if (OrderSymbol() == Symbol() && isFornComment(comment) && profit > 0 && OrderType() == OP_SELL)
        {
            if (TimeHour(OrderOpenTime()) != nowHour || isCurrentDown)
            {
