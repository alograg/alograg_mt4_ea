...
int MAXover(int MAType, int TF, int MAPer1, int MAPer2, int LookBack,
            int MultiXMaxX) {
  ...
  // assign the vars
  MA1_CurrVal = iMA(NULL, TF, MAPer1, 0, MAType, PRICE_CLOSE,
                    1); // most recently closed candle
  MA2_CurrVal = iMA(NULL, TF, MAPer2, 0, MAType, PRICE_CLOSE, 1);
  MA1_PrevVal = iMA(NULL, TF, MAPer1, 0, MAType, PRICE_CLOSE,
                    2); // 1st prior to most recently closed candle
  ...
  if (PrevLoc == -1 && CurrLoc == 1)
    ...
    XOver = 0;
  if (XOver != 0 && LookBack > 0) // check the lookbacks for no XOver - MA must
                                  // be on same side for last "lookback" bars
  {
    XCount = 0;
