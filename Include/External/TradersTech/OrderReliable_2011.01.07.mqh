...
int OrderSendReliable(string symbol, int cmd, double volume, double price,
  ...
  if (bridge == true) {
    ...
    //      Print("bridge = ",bridge," Ln 172, stoploss = ",stoploss,",
    // takeprofit = ",takeprofit);
    ticket = OrderReSendReliable(symbol, cmd, volume, price, slippage, 0, 0,
  ...
  if ((cmd == OP_BUYSTOP) || (cmd == OP_SELLSTOP) || (cmd == OP_BUYLIMIT) ||
      (cmd == OP_SELLLIMIT)) {
    cnt = 0;
    while (!exit_loop) {
      if (IsTradeAllowed()) {
    ...
    if (err == ERR_NO_ERROR) {
      // OrderReliablePrint("apparently successful OP_BUYSTOP or OP_SELLSTOP
      // order placed, details follow.");
      // OrderSelect(ticket, SELECT_BY_TICKET, MODE_TRADES);
      // OrderPrint();
      return (ticket); // SUCCESS!
    }
    if (!limit_to_market) {
      OrderReliablePrint("failed to execute stop or limit order after " +
                         (string)cnt + " retries");
      OrderReliablePrint("failed trade: " + OrderReliable_CommandString(cmd) +
                         " " + (string)symbol + "@" + (string)price + " tp@" +
                         (string)takeprofit + " sl@" +
                         (string)(string)stoploss);
  ...
  if (limit_to_market) {
    OrderReliablePrint(
        "going from limit order to market order because market is too close.");
  ...
  if ((cmd == OP_BUY) || (cmd == OP_SELL)) {
    ...
    while (!exit_loop) {
      if (IsTradeAllowed()) {
        ...
        if (bridge == false) {
          ticket =
              OrderSend(symbol, cmd, volume, price, slippage, stoploss,
                        takeprofit, comment, magic, expiration, arrow_color);
          err = GetLastError();
          _OR_err = err;
        }
        if (err == ERR_INVALID_STOPS) {
          //               Print("This broker uses a bridge, bridge = ",bridge,"
          // Ln 382");
          bridge = true;
          //               Print("bridge = ",bridge," Ln 384, stoploss =
          // ",stoploss,", takeprofit = ",takeprofit);
          ticket =
              OrderReSendReliable(symbol, cmd, volume, price, slippage, 0, 0,
                                  comment, magic, expiration, arrow_color);
      ...
      if (!exit_loop) {
        RefreshRates();
        OrderReliablePrint(
            "retryable error (" + (string)cnt + "/" + (string)retry_attempts +
            "): " + OrderReliableErrTxt(err) + " (price = " + (string)price +
            ", Ask = " + (string)Ask + ", and Bid = " + (string)Bid + ")");
        OrderReliable_SleepRandomTime(sleep_time, sleep_maximum);
        RefreshRates();
        if (cmd == OP_BUY) {
          price = Ask;
    ...
    // we have now exited from loop.
    if (err == ERR_NO_ERROR) {
      // OrderReliablePrint("apparently successful OP_BUY or OP_SELL order
      // placed, details follow.");
      // OrderSelect(ticket, SELECT_BY_TICKET, MODE_TRADES);
      // OrderPrint();
      return (ticket); // SUCCESS!
    }
    OrderReliablePrint("failed to execute OP_BUY/OP_SELL, after " +
                       (string)cnt + " retries");
    OrderReliablePrint("failed trade: " + OrderReliable_CommandString(cmd) +
                       " " + (string)symbol + "@" + (string)price + " tp@" +
                       (string)takeprofit + " sl@" + (string)(string)stoploss);
    OrderReliablePrint("failed to execute OP_BUY/OP_SELL, after " +
                       (string)cnt + " retries");
    OrderReliablePrint("last error: " + OrderReliableErrTxt(err) +
...
bool OrderModifyReliable(int ticket, double price, double stoploss,
  ...
  if (false) {
    ...
    currentOrder = OrderSelect(ticket, SELECT_BY_TICKET, MODE_TRADES);
    int digits = (int)MarketInfo(symbol, MODE_DIGITS);
    if (digits > 0) {
      price = NormalizeDouble(price, digits);
      stoploss = NormalizeDouble(stoploss, digits);
  ...
  if ((result == true) || (err == ERR_NO_ERROR)) {
    // OrderReliablePrint("apparently successful modification order, updated
    // trade details follow.");
    // OrderSelect(ticket, SELECT_BY_TICKET, MODE_TRADES);
    // OrderPrint();
    return (true); // SUCCESS!
  }
  ...
  if (err == ERR_NO_RESULT) {
    OrderReliablePrint(
        "Server reported modify order did not actually change parameters.");
    OrderReliablePrint("redundant modification: " + (string)ticket + " " +
                       (string)symbol + "@" + (string)price + " tp@" +
                       (string)takeprofit + " sl@" + (string)stoploss);
    OrderReliablePrint("Suggest modifying code logic to avoid.");
    return (true);
  }

  OrderReliablePrint("failed to execute modify after " + (string)cnt +
                     " retries");
  OrderReliablePrint("failed modification: " + (string)ticket + " " + symbol +
                     "@" + (string)price + " tp@" + (string)takeprofit +
                     " sl@" + (string)stoploss);
  OrderReliablePrint("last error: " + OrderReliableErrTxt(err));
...
bool OrderModifyReliableSymbol(string symbol, int ticket, double price,
  ...
  return (OrderModifyReliable(ticket, price, stoploss, takeprofit, expiration,
                              arrow_color));
}
...
bool OrderCloseReliable(int ticket, double lots, double price, int slippage,
  ...
  while (!exit_loop) {
    ...
    if (exit_loop) {
      ...
      if ((err != ERR_NO_ERROR) && (err != ERR_NO_RESULT))
        OrderReliablePrint("non-retryable error: " + OrderReliableErrTxt(err) +
                           " (price = " + (string)price + ", Ask = " +
                           (string)Ask + ", and Bid = " + (string)Bid + ")");
  ...
  RefreshRates();
  OrderReliablePrint("failed to execute close after " + (string)cnt +
                     " retries");
...
int OrderReliableLastErr() { return (_OR_err); }
...
//
//=============================================================================
void OrderReliable_EnsureValidStop(int cmd, string symbol, double &price,
                                   double &sl) {
  // Return if no S/L
  if (sl == 0)
    return;
  ...
  if (MathAbs(price - sl) <= servers_min_stop) {
    ...
    if (price > sl)
      ...
      OrderReliablePrint(
          "EnsureValidStop: error, passed in price == sl, cannot adjust");
  ...
  if ((cmd == OP_BUY) || (cmd == OP_SELL)) {
    ...
    while (!exit_loop) {
      if (IsTradeAllowed()) {
        ...
        ticket = OrderSend(symbol, cmd, volume, price, slippage, 0, 0, comment,
