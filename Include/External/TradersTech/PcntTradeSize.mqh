...
double PcntTradeSize(string symbol, int PipRisk, double Pcnt, int NumOfPos = 1,
                     bool FixedAmt = false, int Diagnose = 1) {
  double AcctAmt;
  double TradeSize;
  ...
  if (Diagnose > 1)
    Print("---PcntTradeSize() AcctAmt=", AcctAmt, ", Multiplier=", Multiplier,
          ", PipRisk=", PipRisk);
  if (!FixedAmt)
    TradeSize = AcctAmt * Multiplier / PipRisk /
                (MarketInfo(symbol, MODE_TICKVALUE) * FiveDig);
  else
  ...
  return (TradeSize);
} // double PcntTradeSize(string symbol,int PipRisk,double Pcnt,int
  // NumOfPos=1,bool FixedAmt=false,bool Diagnose=false)