...
int OrderSendReliable(string symbol, int cmd, double volume, double price,
  ...
  OrderReliablePrint("attempted " + OrderReliable_CommandString(cmd) + " " +
                     volume + " lots @" + price + " sl:" + stoploss + " tp:" +
                     takeprofit);
  // limit/stop order.
  int ticket = -1;
  ...
  if ((cmd == OP_BUYSTOP) || (cmd == OP_SELLSTOP) || (cmd == OP_BUYLIMIT) ||
    ...
    if (!limit_to_market) {
      OrderReliablePrint("failed to execute stop or limit order after " + cnt +
                         " retries");
      OrderReliablePrint("failed trade: " + OrderReliable_CommandString(cmd) +
                         " " + symbol + "@" + price + " tp@" + takeprofit +
                         " sl@" + stoploss);
  ...
  if ((cmd == OP_BUY) || (cmd == OP_SELL)) {
    ...
    while (!exit_loop) {
      ...
      if (!exit_loop) {
        RefreshRates();
        OrderReliablePrint("retryable error (" + cnt + "/" + retry_attempts +
                           "): " + OrderReliableErrTxt(err) + " (price = " +
                           price + ", Ask = " + Ask + ", and Bid = " + Bid +
                           ")");
        OrderReliable_SleepRandomTime(sleep_time, sleep_maximum);
        RefreshRates();
        if (cmd == OP_BUY) {
...
bool OrderCloseReliable(int ticket, double lots, double price, int slippage,
  ...
  OrderReliablePrint("failed close: Ticket #" + ticket + ", Price: " + price +
                     ", Slippage: " + slippage);
